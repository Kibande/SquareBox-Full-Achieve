...
void Flesh::check_idle(bool change, bool condition){
	if((speed.x == 0 && on_floor && !is_holding[DOWN_BUTTON] && !is_holding[BLOCK_BUTTON]) || condition){
...
void Flesh::check_idle_atk_front(bool change, bool condition){
	if((pressed[ATTACK_BUTTON] && (is_holding[LEFT_BUTTON] || is_holding[RIGHT_BUTTON])) || condition){
		speed.y = 0;
		if(change) temporary_state = FighterState::IDLE_ATK_FRONT;
		if(!condition) orientation = is_holding[LEFT_BUTTON] ? Orientation::LEFT : Orientation::RIGHT;
	}
}
...
void Flesh::check_jump_atk_neutral(bool change){
	if(is_holding[ATTACK_BUTTON] && !is_holding[DOWN_BUTTON] && !is_holding[UP_BUTTON]){
