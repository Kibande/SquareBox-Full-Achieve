			...
			//	if (layer.tiling != SquareBox::LayerTilingEnum::None) {
				...
				for (unsigned int k = 0; k < layer.world_clusters.size(); k++)
				{
					...
					for (unsigned int l = 0; l < cwcobj.cluster_objects.size(); l++)
					{
						...
						for (unsigned int m = 0; m < ccobj->joints.size(); m++)
						{
							const SquareBox::GWOM::Joint * current_joint = ccobj->joints[m];
				...
				for (unsigned int j = 0; j < fb_layer_world_clusters->size(); j++)
				{
					...
					for (unsigned int k = 0; k < current_fb_world_cluster_cluster_objects->size(); k++)
					{
						...
						for (unsigned int l = 0; l < fb_joints->size(); l++)
						{
							...
							//adding this joint to the layers active joints database
							layer.active_joints_body_a_objects.push_back(jointMemory->body_a_coordinates);
						}
