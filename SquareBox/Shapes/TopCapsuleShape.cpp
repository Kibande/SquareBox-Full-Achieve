...
namespace SquareBox {
	namespace Shapes {
		void TopCapsuleShape::draw(const SquareBox::GWOM::ClusterObject & clusterObject_, SquareBox::RenderEngine::SpriteBatch & spriteBatch_, int opacity_)
		{
			if (!clusterObject_.is_hidden) {
				glm::vec2 boxDims = glm::vec2(clusterObject_.width, clusterObject_.height);
				glm::vec2  boxOrigin = glm::vec2(clusterObject_.position) - boxDims * 0.5f;
				boxOrigin.y += clusterObject_.width*0.25f;
		...
		void TopCapsuleShape::keepTrackOfPosition(SquareBox::GWOM::ClusterObject & clusterObject_)
		{
		...
		void TopCapsuleShape::debugDraw(const SquareBox::GWOM::ClusterObject & clusterObject_, SquareBox::RenderEngine::DebugRenderer & debugRender_, const  SquareBox::RenderEngine::ColorRGBA8 borderColor_)
		{
			const glm::vec2 & boxDims = glm::vec2(clusterObject_.width, clusterObject_.height - clusterObject_.width*0.5f);
			const glm::vec2 & boxOrigin = glm::vec2(clusterObject_.position) - boxDims * 0.5f;
			debugRender_.drawCapsule(glm::vec4(boxOrigin, clusterObject_.width, clusterObject_.height), borderColor_, clusterObject_.angle, false, true);
		}

		void TopCapsuleShape::traceDraw(const SquareBox::GWOM::ClusterObject & clusterObject_, SquareBox::RenderEngine::DebugRenderer & debugRender_, const  glm::vec2 & mouseinworld, const  SquareBox::RenderEngine::ColorRGBA8 & borderColor_, const  float cameraScale_)
		{
			const glm::vec2 & boxDims = glm::vec2(clusterObject_.width, clusterObject_.height - clusterObject_.width*0.5f);
			const glm::vec2 & mouseBorderCenter = mouseinworld - boxDims * 0.5f;
			debugRender_.drawCapsule(glm::vec4(mouseBorderCenter, clusterObject_.width, clusterObject_.height), borderColor_, clusterObject_.angle, false, true);
		}

		void TopCapsuleShape::scalingDraw(SquareBox::GWOM::ClusterObject & clusterObject_, SquareBox::RenderEngine::DebugRenderer & debugRender_, const glm::vec2 & mouseinworld_, SquareBox::InputManager::IInputDevice * input_)
		...
		bool TopCapsuleShape::containtsPoint(SquareBox::GWOM::ClusterObject & clusterObject_, const  glm::vec2 & coordinates_)
		{
		...
		glm::vec2 TopCapsuleShape::localCoordinatesToWorldCoordinates(const SquareBox::GWOM::ClusterObject & clusterObject_, const  glm::vec2 & localCoordinates_)
		{
