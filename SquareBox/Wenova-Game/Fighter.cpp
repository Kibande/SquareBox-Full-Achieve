...
void Fighter::process_input(){
	if(!is("game_over")){
		...
		for(ii button : joystick_buttons){
			pressed[button.first] = alive && input_manager->joystick_button_press(button.second, id);
			is_holding[button.first] = alive && input_manager->is_joystick_button_down(button.second, id);
			released[button.first] = alive && input_manager->joystick_button_release(button.second, id);
		}
	}else{
...
void Fighter::update(float delta){
	...
	box.x += speed.x * delta;
	if(!on_floor) box.y += speed.y * delta;
...
bool Fighter::is_dead(){
	...
	if(dead && partner) partner->set_partner(nullptr);
...
void Fighter::change_state(FighterState cstate){
	...
	if((state == FALLING || state == JUMPING) && (cstate == IDLE || cstate == RUNNING))
		land_sound.play();
	state = cstate;
	Vector csize;
	if(cstate == CROUCH || cstate == CROUCH_ATK || cstate == SPECIAL_1 || cstate == JUMP_ATK_DOWN_FALLLOOP || cstate == JUMP_ATK_DOWN_DMG) csize = crouching_size;
