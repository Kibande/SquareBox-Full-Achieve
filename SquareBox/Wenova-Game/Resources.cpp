...
shared_ptr<SDL_Texture> Resources::get_image(SDL_Renderer* renderer, string file){
	if(image_table.find(file) == image_table.end()){

		auto imageData = SquareBox::AssetManager::IOManager::getPixelDataFromImageFile(file.c_str());

		Uint32 rmask = 0x000000ff;
		Uint32 gmask = 0x0000ff00;
		Uint32 bmask = 0x00ff0000;
		Uint32 amask = 0xff000000;
		SDL_Surface* image_surface = SDL_CreateRGBSurfaceFrom(imageData.pixels, imageData.width, imageData.height, 32, imageData.width * 4, rmask, gmask, bmask, amask);

		SDL_Texture* tx = SDL_CreateTextureFromSurface(renderer,image_surface);
